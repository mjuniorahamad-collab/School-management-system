...
import type { HomeworkFormPayload, TaskStatus } from "@/types/homework"
...
export interface HomeworkFormError {
  field: keyof HomeworkFormValue
  message: string
}

const DATE_RE = /^\d{4}-\d{2}-\d{2}$/

export function defaultHomeworkForm(): HomeworkFormValue {
  return {
    academicSessionId: "",
    classId: "",
    sectionId: "",
...
export function validateHomeworkForm(value: HomeworkFormValue): HomeworkFormError[] {
  const errors: HomeworkFormError[] = []
...
export function homeworkFormToPayload(value: HomeworkFormValue): HomeworkFormPayload {
...
export function homeworkCreatableStatuses(): TaskStatus[] {
