...
export function TeachersTable({ items, isPending, isError, canEdit, onRetry, onEdit }: TeachersViewProps) {
  const navigate = useNavigate()
  if (isPending) return <TeachersSkeleton table />
  if (isError) return <ErrorSkeleton text="Could not load teachers." onRetry={onRetry} />
          ...
          <tbody className="divide-y">
            {items.map((teacher) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <TeacherStatusBadge status={teacher.status as "ACTIVE" | "INACTIVE" | "ON_LEAVE"} />
                </td>
                <td className="whitespace-nowrap px-4 py-3 text-muted-foreground tabular-nums">
                  {formatFullDate(teacher.joiningDate)}
                </td>
...
export function TeachersCards({ items, isPending, isError, canEdit, onRetry, onEdit }: TeachersViewProps) {
  const navigate = useNavigate()
  if (isPending) return <TeachersSkeleton table={false} />
  if (isError) return <ErrorSkeleton text="Could not load teachers." onRetry={onRetry} />
  if (items.length === 0) return <EmptyState />
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center gap-1 rounded-lg border border-dashed py-16 text-center">
      ...
      <p className="text-sm text-muted-foreground">Try adjusting the search, or add a new teacher.</p>
    </div>
  )
}
...
function TeachersSkeleton({ table }: { table: boolean }) {
  return (
    <div
