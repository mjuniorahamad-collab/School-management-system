...
interface HomeworkViewProps {
  ...
  canDelete: boolean
  onRetry: () => void
  onView: (homework: HomeworkListItem) => void
  onEdit: (homework: HomeworkListItem) => void
  onPublish: (homework: HomeworkListItem) => void
  onDelete: (homework: HomeworkListItem) => void
}

export function HomeworkTable(props: HomeworkViewProps) {
  if (props.isPending) return <HomeworkSkeleton table />
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y">
            {props.items.map((homework) => (
              <tr
                key={homework.id}
                className="cursor-pointer transition-colors hover:bg-muted/40"
                onClick={() => props.onView(homework)}
              >
                <td className="px-4 py-3">
                  <p className="font-medium text-foreground">{homework.title}</p>
                  <p className="text-xs text-muted-foreground">{homework.subjectCode} · {homework.subjectName}</p>
                </td>
                <td className="whitespace-nowrap px-4 py-3 text-muted-foreground">
                  {homework.className}
                  {homework.sectionName ? ` · ${homework.sectionName}` : ""}
                </td>
                <td className="whitespace-nowrap px-4 py-3 text-muted-foreground">{homework.teacherName}</td>
                ...
                <td className="px-4 py-3">
                  <TaskStatusBadge status={homework.status} />
                </td>
                {(props.canEdit || props.canPublish || props.canDelete) && (
                  <td className="px-4 py-3">
                    <RowActions {...props} homework={homework} />
                  </td>
                )}
              </tr>
            ))}
...
export function HomeworkCards(props: HomeworkViewProps) {
  if (props.isPending) return <HomeworkSkeleton table={false} />
  if (props.isError) return <ErrorState onRetry={props.onRetry} />
  if (props.items.length === 0) return <EmptyState />
          ...
          >
            <span className="flex items-start gap-3">
              <span className="min-w-0 flex-1">
                <span className="flex items-center gap-2">
                  <span className="truncate text-sm font-medium text-foreground">{homework.title}</span>
                  <TaskStatusBadge status={homework.status} />
                </span>
                <span className="mt-1 block truncate text-xs text-muted-foreground">
                  {homework.subjectCode} · {homework.className}
                  {homework.sectionName ? ` · ${homework.sectionName}` : ""}
                </span>
                <span className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                  <span className="tabular-nums">Due {formatFullDate(homework.dueDate)}</span>
                  {homework.isOverdue && homework.status !== "ARCHIVED" && (
                    <Badge
                      variant="secondary"
                      className="border-transparent bg-red-50 font-medium text-red-700 dark:bg-red-500/15 dark:text-red-300"
    ...
    <div className="flex items-center justify-end gap-1">
      {canPublish && isDraft && (
        <Button
          ...
          variant="ghost"
          size="sm"
          onClick={(e) => { e.stopPropagation(); onPublish(homework) }}
          aria-label={`Publish ${homework.title}`}
      ...
      {canEdit && (
        <Button
          ...
          variant="ghost"
          size="sm"
          onClick={(e) => { e.stopPropagation(); onEdit(homework) }}
      ...
      {canDelete && (
        <Button
          ...
          variant="ghost"
          size="sm"
          onClick={(e) => { e.stopPropagation(); onDelete(homework) }}
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center gap-1 rounded-lg border border-dashed py-16 text-center">
      ...
      <p className="text-sm text-muted-foreground">Create homework to assign it to a class and subject.</p>
    </div>
  )
}
...
function ErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed py-16 text-center">
      <p className="text-sm text-muted-foreground">Could not load homework.</p>
      <button
        type="button"
        onClick={onRetry}
...
function HomeworkSkeleton({ table }: { table: boolean }) {
  return (
    <div
