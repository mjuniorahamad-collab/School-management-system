...
import { useAssignmentsContext, useCreateAssignment, useUpdateAssignment } from "@/hooks/useAssignments"
...
} from "@/lib/assignmentsFormRules"
import type { AssignmentFormValue } from "@/lib/assignmentsFormRules"
...
interface AssignmentsFormDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  editing: AssignmentListItem | null
}

export function AssignmentsFormDialog({ open, onOpenChange, editing }: AssignmentsFormDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {editing
              ? "Update the assignment targeting, details, and lifecycle."
              : "Assign work to a class, subject, and teacher."}
          </DialogDescription>
        </DialogHeader>
        <AssignmentsFormInner key={editing?.id ?? "new"} editing={editing} onOpenChange={onOpenChange} />
      </DialogContent>
    </Dialog>
  )
}

function AssignmentsFormInner({
  editing,
  onOpenChange,
}: {
  editing: AssignmentListItem | null
  ...
  const [form, setForm] = useState<AssignmentFormValue>(() => {
    const base = defaultAssignmentForm()
    if (editing) {
      return {
        ...base,
  ...
  const { data: context } = useAssignmentsContext()
  ...
  const updateMutation = useUpdateAssignment(editing?.id ?? "")
  const isSaving = createMutation.isPending || updateMutation.isPending

  const setField = <K extends keyof AssignmentFormValue>(field: K, value: AssignmentFormValue[K]) => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    const effective: AssignmentFormValue = {
      ...form,
      academicSessionId: effectiveSessionId,
      teacherId: effectiveTeacherId,
    }
    const errors = validateAssignmentForm(effective)
    if (errors.length > 0) {
      toast.error(errors[0].message)
      return
    }
    const payload = assignmentFormToPayload(effective)
    if (editing) {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <Field label="Title" required>
        <Input
          ...
          placeholder="e.g. Term paper draft"
        />
      </Field>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <Field label="Status" required>
          <Select value={form.status} onValueChange={(value) => setField("status", value as AssignmentFormValue["status"])}>
            <SelectTrigger className="w-full">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      <DialogFooter className="gap-2 sm:gap-0">
        ...
        <Button type="submit" disabled={isSaving}>
          {isSaving ? "Saving…" : editing ? "Save changes" : "Create assignment"}
        </Button>
      </DialogFooter>
    </form>
  )
