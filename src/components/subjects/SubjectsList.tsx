...
export function SubjectsTable({ items, isPending, isError, canEdit, onRetry, onEdit }: SubjectsViewProps) {
  if (isPending) return <SubjectsSkeleton table />
  if (isError) return <ErrorSkeleton text="Could not load subjects." onRetry={onRetry} />
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y">
            {items.map((subject) => (
              <tr key={subject.id} className="transition-colors hover:bg-muted/40">
...
export function SubjectsCards({ items, isPending, isError, canEdit, onRetry, onEdit }: SubjectsViewProps) {
  if (isPending) return <SubjectsSkeleton table={false} />
  if (isError) return <ErrorSkeleton text="Could not load subjects." onRetry={onRetry} />
  if (items.length === 0) return <EmptyState />
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center gap-1 rounded-lg border border-dashed py-16 text-center">
      ...
      <p className="text-sm text-muted-foreground">Try adjusting the search, or create a new subject.</p>
    </div>
  )
}
...
function SubjectsSkeleton({ table }: { table: boolean }) {
  return (
    <div
