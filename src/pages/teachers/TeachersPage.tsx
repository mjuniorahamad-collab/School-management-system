...
import { TeachersTable, TeachersCards } from "@/components/teachers/TeachersList"
...
export function TeachersPage() {
  ...
  const { data, isPending, isError, refetch } = useTeachers(query)
    ...
    <PageContainer>
      <div className="flex flex-col gap-4">
        ...
        <TeachersTable
          ...
          onEdit={(teacher) => { setEditing(teacher); setDialogOpen(true) }}
        />
        <TeachersCards
          items={items}
          isPending={isPending}
        ...
        {items.length > 0 && (
          <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
            <span>
              Page {page} of {totalPages} · {total} teacher{total !== 1 ? "s" : ""}
            </span>
        ...
        <TeacherFormDialog open={dialogOpen} onOpenChange={setDialogOpen} teacher={editing} />
      </div>
    </PageContainer>
  )
}
