...
export const createNoticeHandler: RequestHandler = async (req, res) => {
  ...
  const created = await noticeService.createNotice(input, schoolId, actorId)
...
export const updateNoticeHandler: RequestHandler = async (req, res) => {
  ...
  const { schoolId, actorId } = requireAuth(req)
  const updated = await noticeService.updateNotice(routeParam(req.params.id), input, schoolId, actorId)
  res.json(ok(updated))
}
